...
elseif numel(n) == 1
    ...
    if length(population)~=n
       error('POPULATION must be a vector.');
    end
end
...
elseif ~isempty(w)
    if length(w) ~= n
        if isempty(population)
            error('W must have length equal to N.');
        else
            error('W must have the same length as the population.');
        end
    else
        p = w(:)' / sum(w);
    end
...
case {false, 'false', 0}
    if k > n
        ...
        'K must be less than or equal to N for sampling without replacement.');
            ...
            error('K must be less than or equal to the population size.');
        end
    end
    ...
    if isempty(w)
        ...
        error('Weighted sampling without replacement is not supported.');
    end
otherwise
    error('REPLACE must be either true or false.');
end
